...
type Props = { orgId: string; currentUser: User; workspaceId?: string; projectId?: string; canAnnounce: boolean; canManage?: boolean }
...
export function PinsAndAnnouncements({ orgId, currentUser, workspaceId, projectId, canAnnounce, canManage = true }: Props) {
  ...
  async function createPin() { if (!canManage || !pinTitle.trim()) return; await db.pins.add({ id: crypto.randomUUID(), orgId, ...(workspaceId ? { workspaceId } : { projectId }), kind: pinKind, title: pinTitle.trim(), description: pinDescription.trim() || undefined, url: pinUrl.trim() || undefined, createdById: currentUser.id, createdAt: new Date().toISOString() }); setPinOpen(false); resetPin() }
  async function createAnnouncement() { if (!canAnnounce || !title.trim() || !content.trim()) return; await db.announcements.add({ id: crypto.randomUUID(), orgId, ...(workspaceId ? { workspaceId } : { projectId }), title: title.trim(), content: content.trim(), attachments: attachments.split('\n').map((x) => x.trim()).filter(Boolean), links: links.split('\n').map((x) => x.trim()).filter(Boolean), publishAt: publishAt ? new Date(publishAt).toISOString() : new Date().toISOString(), expiresAt: expiresAt ? new Date(expiresAt).toISOString() : undefined, audience, pinned, createdById: currentUser.id, createdAt: new Date().toISOString() }); setAnnouncementOpen(false); resetAnnouncement() }
  return <section className="space-y-8 p-6" aria-label={`${scope} pins and announcements`}>
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="flex items-center gap-2 text-lg font-semibold"><Pin className="size-5 text-primary" />Pinned & announcements</h2><p className="mt-1 text-sm text-muted-foreground">Keep important updates and resources visible to the right people.</p></div><div className="flex flex-wrap gap-2">{canManage && <Button variant="outline" onClick={() => setPinOpen(true)}><Pin /> Pin item</Button>}{canAnnounce && <Button onClick={() => setAnnouncementOpen(true)}><Megaphone /> New announcement</Button>}</div></div>
